...
const PI = math.PI
const MAX_ENERGY_LEVELS = 10

type interval struct{
	// interval for potential eigen value solution
	low, high float64
 }

type  RadialWaveFunction struct {
	N  int
	h  int          
	Rmax float64
	Sol       []float64
	...
	Veff      []float64  // Total Potential	
	Veff0     []float64  
	K         []float64
	EnergyBrackets []interval
	EnergyLevels int
	E   float64
    Z   int
}
...
func NewRadialWaveFunction(N, Z int, Rmax float64) *RadialWaveFunction {
	h := Rmax/float64(N)
	r := make([]float64, N+1)
	for i:=0; i<=N; i++	{
		r[i] = float64(i)*h	
	}        
	psi := RadialWaveFunction{
		N: N,
		h: h,
		Z: Z,
		r   : r,
		...
		Vcoulomb : make( []float64, N+1) 
		Vcentrifugal : make( []float64, N+1) 
		Veff : make( []float64, N+1) 
		Veff0 : make( []float64, N+1) 		
		K    : make( []float64, N+1) 
		EnergyBrackets: make([]interval, MAX_ENERGY_LEVELS)
	}
	psi.setCoulombPotential()
	return &psi
}

func ( w *RadialWaveFunction ) setCoulombPotential(){
	for i:=0; i<=N; i++	{		
		w.Vcoulomb[i] = -float64(w.Z)/w.r[i]
	}        
}
